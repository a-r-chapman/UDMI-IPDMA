...
for (time in times) {
        for (outcome in outcomes) {
                for (model in models) {
                        ...
                        file_path <- glue("Tables/cs/{time}/combined_csHR_sa_{outcome}_{time}_{model}.csv")
                        ...
                        if (length(meta_rows) > 0) {
                                ...
                                var_name <- glue("combined_cshr_sa_{time}_{outcome}_{model}")
                                
                                # Save to global environment
                                assign(var_name, result_df, envir = .GlobalEnv)
                                
                                # Save to file
                                out_file <- glue("Tables/cs/{time}/meta_estimate_csHR_sa_{outcome}_{time}_{model}.csv")
...
assign("res_list_named_sa", res_list_named, envir = .GlobalEnv)