...
# Author-year lookup table [if journal wants full names]
...
pattern_terms <- unname(term_labels)

# Create folders
for (ma in c("cs", "sh")) {
        for (time in c("1y", "5y")) {
                dir.create(glue("Figures/{ma}_sa/{time}"), recursive = TRUE, showWarnings = FALSE)
        }
}
        ...
        for (time in times) {
                for (outcome in outcomes) {
                        for (model in models) {
                                ...
                                obj_name <- glue("combined_{ma_prefix}_sa_{time}_{outcome}_{model}")
                                if (!exists(obj_name)) {
                                ...
                                df_model <- df_model %>%
                                        ...
                                        mutate(
                                                ...
                                                HR_lower = ci_lower,
                                                HR_upper = ci_upper,
                                                weight_scaled = ifelse(!is.na(weight), rescale(weight, to = c(1.5, 4)), NA),
                                        ...
                                        mutate(
                                                study_display = ifelse(
                                                        type == "Summary",
                                                        "Meta-Estimate",
                                                        ifelse(study %in% names(author_year_lookup), author_year_lookup[study], study)
                                                )
                                        ) %>%
                                ...
                                i2_labels <- df_model %>%
                                        ...
                                        left_join(
                                                df_model %>% group_by(outcome) %>% summarise(n_studies = n_distinct(study)),
                                                by = "outcome"
                                        ) %>%
                                        mutate(
                                                xpos = 0.001,
                                ...
                                p <- ggplot(df_model, aes(x = HR, y = -y_pos)) +
                                        geom_point(aes(size = psize, shape = type, fill = type, colour = type)) +
                                        geom_errorbarh(aes(xmin = HR_lower, xmax = HR_upper), height = 0.15, linewidth = 0.6, colour = "black") +
                                        geom_text(aes(x = x_P, label = P_label, fontface = ifelse(type == "Summary", "bold", "plain")), hjust = 0, size = 3.8, family = "Helvetica") +
                                        geom_text(aes(x = x_HR, label = HR_label, fontface = ifelse(type == "Summary", "bold", "plain")), hjust = 0, size = 3.8, family = "Helvetica") +
                                        scale_y_continuous(
                                                breaks = -df_model$y_pos,
                                                labels = with(df_model, ifelse(type == "Summary", paste0("<b>", study_display, "</b>"), paste0("<i>", study_display, "</i>"))),
                                        ...
                                        scale_shape_manual(values = c("Study" = 15, "Summary" = 18)) +
                                        ...
                                        geom_text(data = i2_labels, aes(x = xpos, y = ypos, label = text, group = outcome), inherit.aes = FALSE, hjust = 0, size = 3.5, family = "Helvetica") +
                                        geom_text(data = header_labels, aes(x = x_P, y = y, label = label_P, group = outcome), inherit.aes = FALSE, hjust = 0, size = 4.1, fontface = "bold", family = "Helvetica") +
                                        geom_text(data = header_labels, aes(x = x_HR, y = y, label = label_HR, group = outcome), inherit.aes = FALSE, hjust = 0, size = 4.1, fontface = "bold", family = "Helvetica") +
                                ...
                                # Save
                                out_path <- glue("Figures/{ma}_sa/{time}/forest_plot_sa_{outcome}_{model}_{time}.png")
                                ggsave(out_path, p, width = 12, height = 8.5)
