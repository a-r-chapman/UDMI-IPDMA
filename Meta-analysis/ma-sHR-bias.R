...
cohorts_to_include <- c("APACE", "ACTION","High-STEACS", "KAROLINSKA", "BACC") # Optional: set to NULL to include all
...
for (time in times) {
        for (outcome in outcomes) {
                for (model in models) {
                        ...
                        file_path <- glue("Tables/sh/{time}/combined_sHR_{outcome}_{time}_{model}.csv")
                        ...
                        var_name <- glue("bias_combined_shr_{time}_{outcome}_{model}")
                        ...
                        # Save results to CSV
                        out_file <- glue("Tables/sh/{time}/bias_meta_estimate_sHR_{outcome}_{time}_{model}.csv")
